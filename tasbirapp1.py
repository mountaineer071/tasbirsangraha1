...
Version: 3.0.0 - Enhanced UI & Performance
Features: Elegant Gallery, Advanced Search, Ratings, Comments, Favorites
"""
...
class Config:
    ...
    # Security
    ALLOWED_EXTENSIONS = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.tiff', '.heic', '.avif'}
...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # Create tables with proper SQL formatting
            cursor.execute("""
...
class PhotoAlbumApp:
    ...
    def render_header(self):
        ...
        <div class="main-header">
            ...
            <div style="margin-top: 1rem;">
                ...
                <span class="badge" style="background: #48bb78;">🖼️ Photo Gallery</span>
                <span class="badge" style="background: #ed8936;">💬 Comments</span>
                <span class="badge" style="background: #4299e1;">⭐ Ratings</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    def render_person_gallery(self, person_id: str):
        ...
        with col2:
            ...
            with sqlite3.connect(self.album_manager.db.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT COUNT(*) FROM album_entries WHERE person_id = ?",
                    (person_id,)
                )
        ...
        if comments:
            for comment in comments:
                ...
                <div style="
                    ...
                    border-left: 4px solid #667eea;
                ">
                    <div style="font-weight: 600; color: #333;">{comment['username']}</div>
                    <div style="color: #555; margin: 0.5rem 0;">{comment['content']}</div>
                    <div style="color: #999; font-size: 0.8rem;">{comment['created_at']}</div>
                </div>
                """, unsafe_allow_html=True)
        else:
    ...
    def render_table_of_contents(self):
        ...
        st.markdown('<div class="section-container">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">📑 People Directory</div>', unsafe_allow_html=True)
